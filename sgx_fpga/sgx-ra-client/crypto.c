...
int key_decrypt_certificate(uint8_t* msg, uint8_t* sig, size_t* slen, EVP_PKEY* key)
{
	...
	*sig = NULL;
	...
	/* verify the digest with ta public key */
	if(1 != EVP_DigestVerifyFinal(mdctx, sig, slen)) goto err;
...
/* ------------------------------ */
/* server produce the certificate */
/* ------------------------------ */
...
char* signMessage(const char* privateKey, unsigned char* plainText) {
  ...
  RSASign(privateRSA, plainText, 40, &encMessage, &encMessageLength);
...
bool verifySignature(const char* publicKey, char* plainText, char* signatureBase64) {
  ...
  bool result = RSAVerifySignature(publicRSA, encMessage, encMessageLength, plainText, 40, &authentic);
  return result & authentic;
}

void produce_cert_client(uint64_t eid, unsigned char* cert)
{
	uint64_t tmp_eid = eid;
	memcpy(cert, &tmp_eid, 8);
	unsigned char digest[32];
	sha256_digest(cert, 8, digest);
	memcpy(cert+8, digest, 32);
}